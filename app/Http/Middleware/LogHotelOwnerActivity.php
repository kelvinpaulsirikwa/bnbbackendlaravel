...
class LogHotelOwnerActivity
{
    ...
    public function handle(Request $request, Closure $next): Response
    {
        ...
        $user = Auth::user();
        if (!$user || !in_array($user->role, self::OWNER_ROLES, true)) {
            return $response;
        }
    ...
    protected function log(Request $request, int $ownerUserId): void
    {
        $route = $request->route();
        ...
        $description = OwnerLogMeta::getDescription($request)
        ...
        $payload = [
            ...
            'subject_type' => OwnerLogMeta::getSubjectType($request),
            'subject_id' => OwnerLogMeta::getSubjectId($request),
            'ip_address' => $request->ip(),
            'user_agent' => $request->userAgent(),
            'request_data' => $requestData,
            'old_values' => OwnerLogMeta::getOldValues($request),
            'new_values' => OwnerLogMeta::getNewValues($request) ?? $this->newValuesFromRequestData($request->method(), $requestData),
        ];

        HotelOwnerLog::create(array_filter($payload, fn ($v) => $v !== null));
    }
