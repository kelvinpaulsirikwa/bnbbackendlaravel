...
@section('content')
    <div class="container-fluid py-4" style="background-color: white; min-height: 100vh;">
        ...
        <div class="card shadow-sm mb-4">
            <div class="card-body">
                <form method="GET" action="{{ route('adminpages.admin-logs.index') }}">
                    <div class="row g-3">
                        <div class="col-md-2">
                            ...
                            <select class="form-select" id="admin_user_id" name="admin_user_id">
                                ...
                                @foreach($adminUsers as $u)
                                    <option value="{{ $u->id }}" {{ request('admin_user_id') == $u->id ? 'selected' : '' }}>
                        ...
                        <div class="col-md-3">
                            ...
                            <input type="text" class="form-control" id="search" name="search"
                                   placeholder="Action, description, route..." value="{{ request('search') }}">
                        </div>
                        <div class="col-md-2 d-flex align-items-end">
                            <button type="submit" class="btn btn-outline-primary me-2">
                                <i class="bx bx-search"></i> Filter
                            </button>
                            <a href="{{ route('adminpages.admin-logs.index') }}" class="btn btn-outline-secondary">Clear</a>
                        </div>
                    </div>
                </form>
            </div>
                                ...
                                @foreach($logs as $log)
                                    <tr>
                                        ...
                                        <td class="align-middle">
                                            @if($log->adminUser)
                                                <span class="fw-semibold">{{ $log->adminUser->username ?? $log->adminUser->useremail }}</span>
                                                <br><small class="text-muted">{{ $log->adminUser->useremail }}</small>
                                            @else
                                        ...
                                        <td class="align-middle text-center">
                                            <a href="{{ route('adminpages.admin-logs.show', $log) }}" class="btn btn-sm btn-outline-info" title="View details"><i class="bx bx-show"></i></a>
                                        </td>
                                    </tr>
                                @endforeach
                            </tbody>
